...
public void OnAllPluginsLoaded()
{
	...
	g_UsingNCRPG = LibraryExists("NCRPG");
}
...
{	
	...
	AutoExecConfig_SetFile("SkillAutoBalance");
	cvar_BalanceAfterNRounds = AutoExecConfig_CreateConVar("sab_balanceafternrounds", "0", "0 = Disabled. Otherwise, after map change balance teams when 'N' rounds pass. Then balance based on team win streaks", _, true, 0.0);
	cvar_BalanceAfterNPlayersChange = AutoExecConfig_CreateConVar("sab_balanceafternplayerschange", "0", "0 = Disabled. Otherwise, balance  teams when 'N' players join/leave the server. Requires sab_balanceafternrounds to be enabled", _, true, 0.0);
	cvar_BalanceEveryRound = AutoExecConfig_CreateConVar("sab_balanceeveryround", "0", "If enabled, teams will be rebalanced at the end of every round", _, true, 0.0, true, 1.0);
	cvar_BlockTeamSwitch = AutoExecConfig_CreateConVar("sab_blockteamswitch", "0", "0 = Don't block. 1 = Block, can join spectate, must rejoin same team. 2 = Block, can't join spectate.", _, true, 0.0, true, 2.0);
	cvar_ChatChangeTeam = AutoExecConfig_CreateConVar("sab_chatchangeteam", "0", "Enable joining teams by chat commands '!join, !play, !j, !p, !spectate, !spec, !s (no picking teams)", _, true, 0.0, true, 1.0);
	cvar_DecayAmount = AutoExecConfig_CreateConVar("sab_decayamount", "1.5", "The amount to subtract from a streak if UseDecay is true. In other words, the ratio of a team's round wins to the opposing team's must be greater than this number in order for a team balance to eventually occur.", _, true, 1.0);
	cvar_DisplayChatMessages = AutoExecConfig_CreateConVar("sab_displaychatmessages", "1", "Allow plugin to display messages in the chat", _, true, 0.0, true, 1.0);
	cvar_ForceBalance = AutoExecConfig_CreateConVar("sab_forcebalance", "0", "Add 'force balance' to 'server commands' in generic admin menu", _, true, 0.0, true, 1.0);
	cvar_ForceJoinTeam = AutoExecConfig_CreateConVar("sab_forcejointeam", "0", "0 = Disabled, 1 = Optional (!settings), 2 = Forced. Force clients to join a team upon connecting to the server. Always enabled if both sab_chatchangeteam and sab_teammenu are disabled", _, true, 0.0, true, 2.0);	cvar_GraceTime = FindConVar("mp_join_grace_time");
	cvar_KeepPlayersAlive = AutoExecConfig_CreateConVar("sab_keepplayersalive", "1", "Living players are kept alive when their teams are changed", _, true, 0.0, true, 1.0);
	cvar_MessageColor = AutoExecConfig_CreateConVar("sab_messagecolor", "white", "See sab_messagetype for info");
	cvar_MessageType = AutoExecConfig_CreateConVar("sab_messagetype", "0", "How this plugin's messages will be colored in chat. 0 = no color, 1 = color only prefix with sab_prefixcolor, 2 = color entire message with sab_messagecolor, 3 = color prefix and message with both sab_prefixcolor and sab_messagecolor", _, true, 0.0, true, 3.0);
	cvar_MinPlayers = AutoExecConfig_CreateConVar("sab_minplayers", "7", "The amount of players not in spectate must be at least this number for a balance to occur", _, true, 2.0);
	cvar_MinStreak = AutoExecConfig_CreateConVar("sab_minstreak", "6", "Amount of wins in a row a team needs before autobalance occurs", _, true, 0.0);
	cvar_Prefix = AutoExecConfig_CreateConVar("sab_prefix", "[SAB]", "The prefix for messages this plugin writes in the server");
	...
	cvar_UseDecay = AutoExecConfig_CreateConVar("sab_usedecay", "1", "If 1, subtract sab_decayamount from a team's streak when they lose instead of setting their streak to 0", _, true, 0.0, true, 1.0);
...
Action Timer_GetSkillLvl(Handle timer, int userId)
{
	int client = GetClientOfUserId(userId);
	if (client != 0 && IsClientInGame(client))
	{
		g_iClientScore[client] = 0.0;
		for (int i = 0; i < NCRPG_GetSkillCount(); ++i)
		{
			if(NCRPG_IsValidSkillID(i))
			{
				g_iClientScore[client] += float(NCRPG_GetSkillLevel(client, i));
				char Name[64];
				NCRPG_GetSkillName(i, Name, sizeof(Name));
				PrintToConsoleAll("%N (%s) = %i",client, Name, NCRPG_GetSkillLevel(client, i));
			}
		}
		PrintToConsoleAll("Total = %0.1f", g_iClientScore[client]);
	}
	return Plugin_Handled;
}
...
void GetScore(int client)
{
	...
	else if (scoreType == TYPE_NCRPG)
	{
		if (g_UsingNCRPG)
		{
			...
			CreateTimer(0.4, Timer_GetSkillLvl, GetClientUserId(client), TIMER_FLAG_NO_MAPCHANGE);
			CreateTimer(0.5, Timer_CheckScore, GetClientUserId(client), TIMER_FLAG_NO_MAPCHANGE);
		}
		else
		{
